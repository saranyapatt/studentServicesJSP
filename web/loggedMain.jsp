...
<%
    ...
    try {
        ...
        PreparedStatement w = c.prepareStatement(sql);
        w.setString(1, username);
        ResultSet w1 = w.executeQuery();
        if (w1.next()) {
            String ema = w1.getString("email");
            if (ema.equals("1")) {
                response.sendRedirect("./login/firstTimeLogin.jsp");
            }
        }
...
<html lang="en">
    ...
    <body>
        <%            PreparedStatement p = c.prepareStatement(sql);
            ...
            if (r.next()) {
                session.setAttribute("Fullname", r.getString("fullname"));
                session.setAttribute("student_id", r.getString("student_id"));
                session.setAttribute("id", r.getString("id_number"));
        ...
        <div class="sidebar shadow">
            ...
            <%
                ...
                <h5 class="my-4 text-white text-center">ID: <%= session.getAttribute("student_id")%></h5>

            <nav>
                <a href="loggedMain.jsp" class="nav-link-custom active">🏠 Home</a>
        ...
        <section>
            ...
            <div class="glass-box shadow-lg">
                ...
                <p class="small mb-0" style="letter-spacing: 2px; opacity: 0.8;">KBTU STUDENT SERVICES</p>
            </div>
                                ...
                                <ul class="list-unstyled mt-3 flex-grow-1">
                                    <%
                                        try {
                                            ...
                                            String sql2 = "SELECT * FROM academic_calendar ORDER BY event_id DESC";
                                            PreparedStatement p2 = c.prepareStatement(sql2);
                                            ResultSet r2 = p2.executeQuery();
                                            while (r2.next()) {
                                                DateTimeFormatter dbFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd");
                                                DateTimeFormatter displayFormat = DateTimeFormatter.ofPattern("d MMMM yyyy", Locale.ENGLISH);

                                                String rawStart = r2.getString("start_date");
                                                String rawEnd = r2.getString("end_date");

                                                String formattedStart = LocalDate.parse(rawStart, dbFormat).format(displayFormat);
                                                String formattedEnd = (rawEnd != null && !rawEnd.isEmpty())
                                                        ? LocalDate.parse(rawEnd, dbFormat).format(displayFormat)
                                                        : "";
